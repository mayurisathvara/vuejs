    ...
    public function summary(Request $request)
    {
        ...
        $payload = Cache::remember($cacheKey, self::DASHBOARD_CACHE_TTL_SECONDS, function () use ($request) {
            ...
            $row = (clone $baseQuery)
                ...
                ->selectRaw("SUM(CASE WHEN LOWER(call_logs.call_type) = 'inbound' AND LOWER(call_logs.call_status) = 'missed' THEN 1 ELSE 0 END) as missed_calls")
                ...
                ->selectRaw("SUM(CASE WHEN LOWER(call_logs.call_type) = 'inbound' AND LOWER(call_logs.call_status) = 'missed' THEN 1 ELSE 0 END) as inbound_missed")
                ->selectRaw("SUM(CASE WHEN LOWER(call_logs.call_type) = 'inbound' AND LOWER(call_logs.call_status) = 'missed' AND call_logs.call_back = 'Y' THEN 1 ELSE 0 END) as missed_returned")
                ->selectRaw("SUM(CASE WHEN LOWER(call_logs.call_type) = 'inbound' AND LOWER(call_logs.call_status) = 'missed' AND call_logs.call_back = 'N' THEN 1 ELSE 0 END) as missed_pending")
